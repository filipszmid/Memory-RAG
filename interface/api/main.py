...
async def extract_facts(request: ExtractRequest):
    ...
    try:
        ...
        return ExtractResponse(job_id=job.get_id(), status="queued")
    except Exception as e:
...
async def get_extraction_status(job_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail="Job not found or error fetching status"
        )
...
async def chat(request: ChatRequest):
    ...
    context, retrieved_facts = req_rag.get_context_for_session(
        user_id=request.user_id, current_message=request.message, config=rag_config
    )
    ...
    try:
        ...
        if proxy_url:
            ...
            litellm_model = (
                ...
                else f"ollama/{request.model}"
            )
            api_base = (
                settings.ollama_base_url if request.provider == "ollama" else None
            )
    ...
    return ChatResponse(
        response=answer,
        context_used=[
            line[2:] for line in context.splitlines() if line.startswith("- ")
        ],
        telemetry={
...
async def reset_index():
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Index successfully reset and recreated with 1536 dims.",
        }
    except Exception as e:
