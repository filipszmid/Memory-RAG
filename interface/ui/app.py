...
with st.sidebar:
    ...
    ds_mode = st.toggle(
        "Data Science Mode",
        value=False,
        help="Enable advanced RAG tuning and observability.",
    )

    st.subheader("Model & Provider")
    sel_provider = st.selectbox(
        "Provider", options=list(PROVIDER_MODELS.keys()), index=0
    )
    ...
    threshold = st.number_input(
        ...
        max_value=100,
        value=20,
        help="Inject full profile if facts < this.",
    )
    ...
    rerank = st.toggle(
        "Enable Reranking",
        value=False,
        help="Use an LLM pass to reorder retrieved facts.",
    )
    if rerank and ds_mode:
        rerank_model = st.selectbox(
            "Rerank Model", options=["gpt-4o-mini", "gemini-1.5-flash", "llama3"]
        )
    else:
    ...
    if st.button(
        ...
        help="Extract facts from the current conversation and save to ElasticSearch.",
    ):
        if "messages" in st.session_state and st.session_state.messages:
            with st.status("Extracting Facts...", expanded=True) as status:
                ...
                job_id = trigger_extraction(
                    user_id, clean_messages, sel_provider, sel_model
                )

                if job_id:
                    status.update(
                        label=f"Job queued (ID: {job_id}). Waiting for worker...",
                        state="running",
                    )
                    ...
                    while not finished:
                        ...
                        if job_status == "finished":
                            ...
                            st.success(
                                f"Successfully processed! Extracted {new_facts_count} facts. (Stored/Updated {stored_count} in ES)"
                            )
                            ...
                            status.update(
                                label=f"Worker is processing memory... (Status: {job_status})"
                            )

                status.update(
                    label="Memory Processing Complete!",
                    state="complete",
                    expanded=False,
                )
        else:
    ...
    if not facts:
        ...
        for f in facts:
            ...
            with col1:
                ...
                st.caption(
                    f"ID: {f['id']} | Confidence: {f['confidence']} | Created: {f['created_at']}"
                )
            with col2:
                if st.button("Delete", key=f"del_{f['id']}"):
                    if delete_fact(f["id"]):
                        ...
                        st.session_state.facts = [
                            x for x in st.session_state.facts if x["id"] != f["id"]
                        ]
                        st.rerun()
            st.divider()
...
if prompt := st.chat_input("Enter your message..."):
    ...
    result = call_chat(
        user_id, prompt, rag_config, sel_provider, sel_model, temperature, top_p
    )
    if result:
