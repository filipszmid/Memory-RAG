...
def mock_pipeline_deps():
    with patch(
        "src.pipeline.extraction.ProviderFactory.get_provider"
    ) as mock_factory, patch(
        "src.pipeline.extraction.ESFactStore"
    ) as mock_store, patch(
        "src.pipeline.extraction.DeduplicationEngine"
    ) as mock_dedup:
...
def test_extract_single_conversation_success(tmp_path, mock_pipeline_deps):
    ...
    mock_pipeline_deps["provider"].generate_facts.return_value = (
        mock_facts_resp,
        {"latency": 0.5},
    )
