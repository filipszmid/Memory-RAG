...
def test_rag_manager_get_context_below_threshold(mock_store, mock_litellm):
    ...
    manager = RAGManager(mock_store)
    context, facts = manager.get_context_for_session(
        "user-1", "What do I do for a living?"
    )
...
def test_rag_manager_get_context_vector_strategy(mock_store, mock_litellm):
    ...
    mock_store.knn_search.return_value = [{"id": "f1", "fact": "Fact 1", "score": 0.9}]

    manager = RAGManager(mock_store)
    context, facts = manager.get_context_for_session(
        "user-1", "Checking fact 1 relevance", config=config
    )
