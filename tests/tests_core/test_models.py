...
def test_extract_request_valid(sample_user_id, sample_messages):
    req = ExtractRequest(
        user_id=sample_user_id, messages=sample_messages, provider="openai"
    )
...
def test_chat_request_defaults(sample_user_id):
    req = ChatRequest(user_id=sample_user_id, message="What is my cat's name?")
