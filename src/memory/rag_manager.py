...
class RAGConfig(BaseModel):
    ...
    top_k: int = 5
    threshold: int = 20
    alpha: float = 0.5  # Weighted balance between vector and keyword
    rerank: bool = False
...
class RAGManager:
    ...
    def __init__(
        self, store: ESFactStore, provider: str = "openai", model: str = "gpt-4o"
    ):
        self.store = store
    ...
    def _get_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            a_base = (
                proxy_url
                if proxy_url
                else (settings.ollama_base_url if self.provider == "ollama" else None)
            )
            a_key = settings.litellm_master_key if proxy_url else None
    ...
    def _rerank_facts(
        self, query: str, facts: List[Dict[str, Any]], config: RAGConfig
    ) -> List[Dict[str, Any]]:
        """
        Reranks facts using LiteLLM when second-pass relevance scoring is enabled.
        """
        ...
        prompt = (
            f"Role: Relevance ranker. User Message: '{query}'\n\n"
            f"Facts:\n{facts_text}\n\n"
        ...
        try:
            ...
            if proxy_url:
                a_base = proxy_url
                a_key = settings.litellm_master_key
    ...
    def get_context_for_session(
        self, user_id: str, current_message: str, config: Optional[RAGConfig] = None
        ...
        if len(all_active) <= config.threshold:
            ...
            if config.strategy == "vector":
                ...
                useful_facts = (
                    self.store.knn_search(user_id, query_vector, top_k=config.top_k)
                    if query_vector
                    else []
                )
            elif config.strategy == "bm25":
                useful_facts = self.store.text_search(
                    user_id, current_message, top_k=config.top_k
                )
            else:  # hybrid
                query_vector = self._get_embedding(current_message)
                if query_vector:
