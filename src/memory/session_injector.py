...
class SessionInjector:
    ...
    def inject_memory(
        self, user_message: str, rag_config: Optional[RAGConfig] = None
        ...
        if msg_len < 5:
            logger.info(
                f"Skipping memory injection for short message: '{user_message}'"
            )
            return "", []
        ...
        if len(self.injected_ids) >= self.max_injections:
            logger.warning(
                f"Injection cap reached ({self.max_injections}). Skipping further updates."
            )
            return "", []
        ...
        if self.is_first_message:
            logger.info(
                f"First message detected for {self.user_id}. Injecting key profile facts."
            )
        ...
        rag_prompt, rag_facts = self.rag_manager.get_context_for_session(
            user_id=self.user_id, current_message=user_message, config=rag_config
        )
