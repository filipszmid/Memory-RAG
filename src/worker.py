...
def process_extraction(
    user_id: str, messages: list, provider: str, model: str, conversation_id: str = None
):
    """
    ...
    try:
        ...
        result = pipeline.extract_facts_from_messages(
            user_id=user_id, messages=messages, conversation_id=conversation_id
        )

        logger.info(
            f"Finished background extraction for user {user_id}. Facts found: {len(result.get('facts', []))}"
        )
        return {
            "status": "success",
