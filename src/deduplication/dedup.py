...
class DeduplicationEngine:
    ...
    def _get_embedding(self, text: str) -> List[float]:
        ...
        try:
            resp = litellm.embedding(
                ...
                input=[text],
                api_base=(
                    settings.ollama_base_url if self.provider == "ollama" else None
                ),
            )
    ...
    def process_new_fact(
        self, user_id: str, fact: Dict[str, Any], conversation_id: str
    ) -> str:
        """
            ...
            if score >= settings.dup_uncertainty:
                ...
                if relationship == "DUPLICATE":
                    self.store.update_fact(
                        ...
                        {
                            "last_confirmed_at": datetime.datetime.now().isoformat(),
                            "confirmation_count": match.get("confirmation_count", 1)
                            + 1,
                        },
                    )
                    return match["id"]
                    ...
                    logger.info(
                        f"Semantic contradiction detected. Superseding '{match['fact']}' with '{fact_text}'"
                    )
                    new_id = self.store.save_fact(user_id, fact)
